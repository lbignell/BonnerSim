...
{  
 ...
 /// Back Fallange for CLYC (BFFC)
  ...
  G4double BLO_radius = 1.285*cm;
  G4double BLI_radius = 1.07*cm;
  G4double BLO_dZ = 20.25*cm; 
  
  G4Material* beamline_mat = nist->FindOrBuildMaterial("G4_STAINLESS-STEEL");
  G4Tubs* beamline = 
    new G4Tubs("beamline", BLI_radius, BLO_radius, BLO_dZ, 0., twopi);
  G4LogicalVolume* logicbeamline =                         
    new G4LogicalVolume(beamline,             //its solid
                        beamline_mat,         //its material
                        "beamline");        //its name
  // Rotation matrix
  G4ThreeVector position2 = G4ThreeVector(0.,22.99*cm,-49.75*cm);
  ...
  //// Beam line internal vacuum (BLIV)

  G4double BLIV_radius = 1.07*cm;
  G4double BLIV_dZ = 20.25*cm; 
  
  G4Tubs* BLIV = 
    new G4Tubs("BLIV", 0.0, BLIV_radius, BLIV_dZ, 0., twopi);
  G4LogicalVolume* logicBLIV =                         
    new G4LogicalVolume(BLIV,             //its solid
                        Vacuum,//BLIV_mat,         //its material
                        "BLIV");        //its name
  // Rotation matrix
  G4ThreeVector position100 = G4ThreeVector(0.,22.99*cm,-49.75*cm);
  G4RotationMatrix rotm100  = G4RotationMatrix();
  rotm100.rotateY(0*deg); 
  rotm100.rotateX(0*deg); 
  rotm100.rotateZ(0*deg); 
  G4Transform3D transform100 = G4Transform3D(rotm100, position100);  
  // Build beam line
    new G4PVPlacement(transform100,                     //no rotation
                    logicBLIV,          //its logical volume
                    "BLIV",             //its name
                    logicWorld,              //its mother  volume
                    false,                   //no boolean operation
                    0,                       //copy number
  ...
  //// Main Chuck
  G4double MCO_radius = 2.534*cm;
  G4double MCO_dZ = 5.01*cm;  
  G4double MCI_radius = 2.356*cm;

  G4Material* mainchunk_mat = nist->FindOrBuildMaterial("G4_STAINLESS-STEEL");
  G4Tubs* mainchunk = 
    new G4Tubs("mainchunk", MCI_radius, MCO_radius, MCO_dZ, 0., twopi);
  G4LogicalVolume* logicmainchunk =                         
    new G4LogicalVolume(mainchunk,             //its solid
                        mainchunk_mat,         //its material
                        "mainchunkLV");        //its name

  // Rotation matrix
  G4ThreeVector position3 = G4ThreeVector(0.,22.99*cm,-27.4*cm);
  G4RotationMatrix rotm3  = G4RotationMatrix();
  rotm3.rotateY(0*deg); 
  rotm3.rotateX(90*deg); 
  rotm3.rotateZ(0*deg); 
  G4Transform3D transform3 = G4Transform3D(rotm3, position3);  
  //// Main Chunk with hole for beam line
  // Build Chunk
  G4ThreeVector BPCTrans = G4ThreeVector(0., -20.0*cm, 0.);
  G4RotationMatrix* BPCRot = new G4RotationMatrix;
    BPCRot->rotateX(90.0*deg);
  G4SubtractionSolid* MCWBL = 
    new G4SubtractionSolid("MCWBL", mainchunk, wholepipe, BPCRot, BPCTrans);
  G4LogicalVolume* logicMCWBL =                         
    new G4LogicalVolume(MCWBL,             //its solid
                        mainchunk_mat,             //its material
                        "MCWBLLV");        //its name
    new G4PVPlacement(transform3,                     //no rotation
                    logicMCWBL,          //its logical volume
                    "MCWBL",             //its name
                    logicWorld,              //its mother  volume
                    false,                   //no boolean operation
                    0,                       //copy number
  ...
  //// Build top fallange (TFL) (built blind)
  // Geometry
  G4double TFL_radius = 4.8*cm;
  G4double TFL_dZ = 1.15*cm;  

  G4Material* TFL_mat = nist->FindOrBuildMaterial("G4_STAINLESS-STEEL");
  G4Tubs* TFL = 
    new G4Tubs("TFL", 0., TFL_radius, TFL_dZ, 0., twopi);
  G4LogicalVolume* logicTFL =                         
    new G4LogicalVolume(TFL,             //its solid
                        TFL_mat,         //its material
                        "TFLLV");        //its name
  
  // Matrix
  G4ThreeVector position4 = G4ThreeVector(0.,29.15*cm,-27.4*cm);
  G4RotationMatrix rotm4  = G4RotationMatrix();
  rotm4.rotateY(0*deg); 
  rotm4.rotateX(90*deg); 
  rotm4.rotateZ(0*deg); 
  G4Transform3D transform4 = G4Transform3D(rotm4, position4);  
  // Place in world
  new G4PVPlacement(transform4,                       //no rotation
                    logicTFL,               //its logical volume
                    "TFL",                  //its name
                    logicWorld,              //its mother  volume
                    false,                   //no boolean operation
                    0,                       //copy number
  ...
  //// Build bottom fallange (BFL) (built blind)
  // Geometry
  G4double BFL_radius = 4.8*cm;
  G4double BFL_dZ = 1.14*cm;  

  G4Material* BFL_mat = nist->FindOrBuildMaterial("G4_STAINLESS-STEEL");
  G4Tubs* BFL = 
    new G4Tubs("BFL", 0., BFL_radius, BFL_dZ, 0., twopi);
  G4LogicalVolume* logicBFL =                         
    new G4LogicalVolume(BFL,             //its solid
                        BFL_mat,         //its material
                        "BFLLV");        //its name
  
  // Matrix
  G4ThreeVector position5 = G4ThreeVector(0.,16.84*cm,-27.4*cm);
  G4RotationMatrix rotm5  = G4RotationMatrix();
  rotm5.rotateY(0*deg); 
  rotm5.rotateX(90*deg); 
  rotm5.rotateZ(0*deg); 
  G4Transform3D transform5 = G4Transform3D(rotm5, position5);  
  // Place in world
  new G4PVPlacement(transform5,                       //no rotation
                    logicBFL,               //its logical volume
                    "BFL",                  //its name
                    logicWorld,              //its mother  volume
                    false,                   //no boolean operation
                    0,                       //copy number
  ...
  //// Vacuum inside target chamber
  //Main chunk inner for Vacuum (MCI4V)
  G4double MCI4V_radius = 2.356*cm;
  G4double MCI4V_dZ = 5.01*cm;  

  G4Tubs* MCI4V = 
    new G4Tubs("MCI4V", 0., MCI4V_radius, MCI4V_dZ, 0., twopi);
  G4LogicalVolume* logicMCI4V =                         
    new G4LogicalVolume(MCI4V,             //its solid
                        Vacuum,         //its material
                        "MCI4VLV");        //its name
  // Rotation matrix
  G4ThreeVector positionMCI4V = G4ThreeVector(0.,22.99*cm,-27.4*cm);
  G4RotationMatrix rotmMCI4V  = G4RotationMatrix();
  rotmMCI4V.rotateY(0*deg); 
  rotmMCI4V.rotateX(90*deg); 
  rotmMCI4V.rotateZ(0*deg); 
  G4Transform3D transformMCI4V = G4Transform3D(rotmMCI4V, positionMCI4V);  
  // Place in world
  new G4PVPlacement(transformMCI4V,                       //no rotation
                    logicMCI4V,               //its logical volume
                    "MCI4V",                  //its name
                    logicWorld,              //its mother  volume
                    false,                   //no boolean operation
                    0,                       //copy number
                    fCheckOverlaps);         // checking overlaps 
  ////Build target thing 
  // Ceramic connector (using aluminium oxide)(CerC)
  // Geometry
  G4double CerC_radius = 0.65*cm;
  G4double CerC_dZ = .78*cm;  

  G4Material* CerC_mat = nist->FindOrBuildMaterial("G4_ALUMINUM_OXIDE");
  G4Tubs* CerC = 
    new G4Tubs("CerC", 0., CerC_radius, CerC_dZ, 0., twopi);
  G4LogicalVolume* logicCerC =                         
    new G4LogicalVolume(CerC,             //its solid
                        CerC_mat,         //its material
                        "CerCLV");        //its name
  
  // Matrix
  G4ThreeVector position7 = G4ThreeVector(0.,0.,-(5.01-0.78)*cm);
  G4RotationMatrix rotm7  = G4RotationMatrix();
  rotm7.rotateY(0*deg); 
  rotm7.rotateX(0*deg); 
  rotm7.rotateZ(0*deg); 
  G4Transform3D transform7 = G4Transform3D(rotm7, position7);  
  // Place in world
  new G4PVPlacement(transform7,                       //no rotation
                    logicCerC,               //its logical volume
                    "CerC",                  //its name
                    logicMCI4V,              //its mother  volume
                    false,                   //no boolean operation
                    0,                       //copy number
                    fCheckOverlaps);         // checking overlaps 
  ...
  //Copper cube (CuCube)
  // Geometry
  G4double CuCube_width = 1.275*cm;
  G4double CuCube_height = 1.356*cm;
  G4double CuCube_depth = 1.275*cm;

  G4Material* CuCube_mat = nist->FindOrBuildMaterial("G4_Cu");
  G4Box* CuCube = new G4Box("CuCube", CuCube_width, CuCube_depth, CuCube_height);  
  G4LogicalVolume* logicCuCube =                         
    new G4LogicalVolume(CuCube,             //its solid
                        CuCube_mat,         //its material
                        "CuCubeLV");        //its name
  
  // Matrix
  G4ThreeVector position8 = G4ThreeVector(0.,0.,-(5.01-2*0.78-1.356)*cm);
  G4RotationMatrix rotm8  = G4RotationMatrix();
  rotm8.rotateY(0*deg); 
  rotm8.rotateX(0*deg); 
  rotm8.rotateZ(0*deg); 
  G4Transform3D transform8 = G4Transform3D(rotm8, position8);  
  // Place in world
  new G4PVPlacement(transform8,                       //no rotation
                    logicCuCube,               //its logical volume
                    "CuCube",                  //its name
                    logicMCI4V,              //its mother  volume
                    false,                   //no boolean operation
                    0,                       //copy number
                    fCheckOverlaps);         // checking overlaps 
  //Copper holder 
  //Small Copper Prism (SCP)
  G4double SCP_width = 0.95*cm;
  G4double SCP_height = 0.95*cm;
  G4double SCP_depth = 0.15*cm;

  G4Material* SCP_mat = nist->FindOrBuildMaterial("G4_Cu");
  G4Box* SCP = new G4Box("SCP", SCP_width, SCP_height, SCP_depth);  
  G4LogicalVolume* logicSCP =                         
    new G4LogicalVolume(SCP,             //its solid
                        SCP_mat,         //its material
                        "SCPLV");        //its name
  //Cylinder (HIC)
  G4double HIC_radius = 0.6*cm;
  G4double HIC_dZ = 0.16*cm;  

  G4Material* HIC_mat = nist->FindOrBuildMaterial("G4_Cu");
  G4Tubs* HIC = 
    new G4Tubs("HIC", 0., HIC_radius, HIC_dZ, 0., twopi);
  G4LogicalVolume* logicHIC =                         
    new G4LogicalVolume(HIC,             //its solid
                        HIC_mat,         //its material
                        "HICLV");        //its name
  // Rotation matrix
  G4ThreeVector position9 = G4ThreeVector(0.,1.125*cm,-(5.01-2*0.78-2*1.356-0.95)*cm);
  G4RotationMatrix rotm9  = G4RotationMatrix();
  rotm9.rotateY(0*deg); 
  rotm9.rotateX(90*deg); 
  rotm9.rotateZ(0*deg); 
  G4Transform3D transform9 = G4Transform3D(rotm9, position9);  
  // Build hollow target holder (HTH)
  G4SubtractionSolid* HTH = 
    new G4SubtractionSolid("HTH", SCP, HIC);
  G4LogicalVolume* logicHTH =                         
    new G4LogicalVolume(HTH,             //its solid
                        HIC_mat,             //its material
                        "HTH");        //its name
    new G4PVPlacement(transform9,                     //no rotation
                    logicHTH,          //its logical volume
                    "HTH",             //its name
                    logicMCI4V,              //its mother  volume
                    false,                   //no boolean operation
                    0,                       //copy number
                    fCheckOverlaps);         // checking overlaps 
  // Tantalum layer (tantalum)
  // Geometry
  G4double tantalum_width = 0.95*cm;
  G4double tantalum_height = 0.95*cm;
  G4double tantalum_depth = .1*cm;

  G4Material* tantalum_mat = nist->FindOrBuildMaterial("G4_Ta");
  G4Box* tantalum = new G4Box("tantalum", tantalum_width, tantalum_depth, tantalum_height);  
  G4LogicalVolume* logictantalum =                         
    new G4LogicalVolume(tantalum,             //its solid
                        tantalum_mat,         //its material
                        "tantalumLV");        //its name
  // Matrix
  G4ThreeVector positiontantalum = G4ThreeVector(0.,(1.275-2*0.15-0.1)*cm,-(5.01-2*0.78-2*1.356-0.95)*cm);
  G4RotationMatrix rotmtantalum  = G4RotationMatrix();
  rotmtantalum.rotateY(0*deg); 
  rotmtantalum.rotateX(0*deg); 
  rotmtantalum.rotateZ(0*deg); 
  G4Transform3D transformtantalum = G4Transform3D(rotmtantalum, positiontantalum);  
  // Place in world
  new G4PVPlacement(transformtantalum,                       //no rotation
                    logictantalum,               //its logical volume
                    "tantalum",                  //its name
                    logicMCI4V,              //its mother  volume
                    false,                   //no boolean operation
                    0,                       //copy number
  ...
  //Target (target)
  // Geometry
  G4double target_width = 0.95*cm;
  G4double target_height = 0.95*cm;
  G4double target_depth = .025*cm;

  G4Material* target_mat = nist->FindOrBuildMaterial("G4_LITHIUM_FLUORIDE");
  G4Box* target = new G4Box("target", target_width, target_depth, target_height);  
  ...
  //Make target into sensitive detector
  SensDTarget* absoSDTarget;
  absoSDTarget = new SensDTarget ( absoSDTargetname );
  //G4SDManager *SDmanager=G4SDManager::GetSDMpointer();  already called in making clyc sensitive
  logictarget->SetSensitiveDetector(absoSDTarget);*/


  // Matrix
  G4ThreeVector position10 = G4ThreeVector(0.,(1.275-2*0.15-2*0.1-.025)*cm,-(5.01-2*0.78-2*1.356-0.95)*cm);
  G4RotationMatrix rotm10  = G4RotationMatrix();
  rotm10.rotateY(0*deg); 
  rotm10.rotateX(0*deg); 
  rotm10.rotateZ(0*deg); 
  G4Transform3D transform10 = G4Transform3D(rotm10, position10);  
  // Place in world
  new G4PVPlacement(transform10,                       //no rotation
  ...
  //Aluminium layer
  //Outer Frame Prism (OFP)
  G4double OFP_width = 0.95*cm;
  G4double OFP_height = 0.95*cm;
  G4double OFP_depth = 0.025*cm;

  G4Material* OFP_mat = nist->FindOrBuildMaterial("G4_Al");
  G4Box* OFP = new G4Box("OFP", OFP_width, OFP_height, OFP_depth);  
  G4LogicalVolume* logicOFP =                         
    new G4LogicalVolume(OFP,             //its solid
                        OFP_mat,         //its material
                        "OFPLV");        //its name

  //Inner Frame Cylinder (IFC)
  G4double IFC_radius = 0.475*cm;
  G4double IFC_dZ = 0.026*cm;  

  G4Material* IFC_mat = nist->FindOrBuildMaterial("G4_Al");
  G4Tubs* IFC = 
    new G4Tubs("IFC", 0., IFC_radius, IFC_dZ, 0., twopi);
  G4LogicalVolume* logicIFC =                         
    new G4LogicalVolume(IFC,             //its solid
                        IFC_mat,         //its material
                        "IFCLV");        //its name
  // Rotation matrix
  G4ThreeVector position11 = G4ThreeVector(0.,(1.275-2*0.15-2*0.1-3*.025)*cm,-(5.01-2*0.78-2*1.356-0.95)*cm);
  G4RotationMatrix rotm11  = G4RotationMatrix();
  rotm11.rotateY(0*deg); 
  rotm11.rotateX(90*deg); 
  rotm11.rotateZ(0*deg); 
  G4Transform3D transform11 = G4Transform3D(rotm11, position11);  
  // Build Aluminium Trap Layer (ALTL)
  G4SubtractionSolid* ALTL = 
    new G4SubtractionSolid("ALTL", OFP, IFC);
  G4LogicalVolume* logicALTL =                         
    new G4LogicalVolume(ALTL,             //its solid
                        IFC_mat,             //its material
                        "ALTL");        //its name
    new G4PVPlacement(transform11,                     //no rotation
                    logicALTL,          //its logical volume
                    "ALTL",             //its name
                    logicMCI4V,              //its mother  volume
                    false,                   //no boolean operation
                    0,                       //copy number
